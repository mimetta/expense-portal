...
## Users & access: the new tables, and what is transitional

Identity and permissions live in `people` / `person_roles` / `bo_scopes` /
`person_menu_overrides` (migration 035). `lib/permissions.ts` delegates to
`lib/access-v2.ts`; role-to-menu defaults are in CODE, and
`person_menu_overrides` holds only per-person exceptions.

**`roles` and `settings_tab_permissions` still exist and are FROZEN.** Nothing
reads or writes them. They are the rollback path — reverting the switch-over
commit restores them as the source of truth with no data migration — and they
are not to be dropped until that path is given up deliberately.

### `scripts/verify-old-vs-new.ts` is TRANSITIONAL. Do not treat it as a gate.

It compares the old logic (a `CurrentUser` built from `roles`, which makes
`lib/permissions.ts` take its legacy branch) against the new logic, to prove
the switch-over changed nobody's access. That was true on the day it was
written and is the reason it exists.

It **loses meaning as legitimate changes accumulate**. `roles` is frozen, so
every real access change made through Settings > Users & access makes the two
diverge — correctly. The script already had to be taught to separate "changed
since the switch" from "regression" after it reported a real admin action
(`panwipa.s` promoted to ACCOUNTING) as a failure. Each further change widens
that skip list, and once it skips everyone the script proves nothing.

**Retire it when `roles` is dropped.** Until then, a non-empty skip list is
expected, not a problem, and anything it skips is not being checked — assert
those cases directly instead, as was done for the three former DEPT_HEAD
holders' Products and Suppliers grants.

**`docs/access-baseline.md` is the durable record.** It is a committed snapshot
of what every person could do, generated by calling the real permission
functions. It is the pass condition for any future change to this area.

### `scripts/backfill-access-v2.ts` refuses to re-run

It was a one-time migration. Re-running `--apply` would DELETE all four tables
and rebuild them from the frozen `roles` table, silently reverting everything
since. It now compares the live tables against what it would produce and
refuses when they differ, naming what would be lost. `--force` exists for a
deliberate re-migration.

### Every write to these tables is audited

| path | audit action |
|---|---|
| `PUT /api/users-access` | `PERSON_ACCESS_UPDATED` (full before/after) |
| `POST /api/users-access` | `PERSON_CREATED` |
| `lib/person.ts#autoRegisterPerson` | `AUTO_REGISTERED` |
| `scripts/backfill-access-v2.ts --apply` | `ACCESS_BACKFILL_APPLIED` |

There are no others. `/api/roles` POST/PATCH/DELETE were retired (410) in
stage 2c precisely because they wrote these tables with no audit row — which
